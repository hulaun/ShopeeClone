...
function Profile() {
  ...
  const [currentTab, setCurrentTab] = useState("mainProfile");
      ...
      <motion.div className="max-w-7xl py-3 m-auto">
        <motion.div className="my-2 flex gap-12">
          ...
          <motion.section className="bg-white rounded-md p-4 w-full border border-grey-200">
            {currentTab === "mainProfile" && <MainProfile />}
            {currentTab === "bankProfile" && <div>Bank Profile</div>}
            {currentTab === "addressProfile" && <div>Address Profile</div>}
            {currentTab === "changePassword" && <div>Change Password</div>}
            {currentTab === "notificationProfile" && <div>Notification Profile</div>}
            {currentTab === "mainNotification" && <div>Main Notification</div>}
            {currentTab === "promotionNotification" && <div>Promotion Notification</div>}
            {currentTab === "systemNotification" && <div>System Notification</div>}
            {currentTab === "shopeeNotification" && <div>Shopee Notification</div>}
            {currentTab === "voucher" && <div>Voucher</div>}
            {currentTab === "orders" && <div>Orders</div>}
          </motion.section>
        </motion.div>
      </motion.div>
      <HomeFooter />
